...
public class NativeInApp {
    ...
    private static NativeInApp nativeInApp;

    public static NativeInApp getInstance() {
        if (nativeInApp == null) {
            nativeInApp = new NativeInApp();
        }
        return nativeInApp;
    }
    ...
    private int getPosLayout(int position) {
        int layout;
        if (position == 0) {
    ...
    public void loadNativeGA(final Activity activity, LinearLayout lnNativeAd, int position, String idAds) {
        try {
                    ...
                    if (InternetUtils.checkInternet(activity)) {
                        ...
                        builder.forNativeAd(new NativeAd.OnNativeAdLoadedListener() {
                            ...
                            public void onNativeAdLoaded(NativeAd nativeAd) {
                                ...
                                NativeAdView nativeAdView = (NativeAdView) activity.getLayoutInflater().inflate(getPosLayout(position), null);
                                populateNativeAdView(nativeAd, nativeAdView);
                                lnNativeAd.removeAllViews();
                                lnNativeAd.addView(nativeAdView);
                            }
                        });
                        ...
                        AdLoader adLoader = builder.withAdListener(new AdListener() {
                            ...
                            public void onAdFailedToLoad(LoadAdError loadAdError) {
                                NativeGAM.getInstance().showNativeGAM(activity, lnNativeAd, 1);
                            }
                        ...
                        lnNativeAd.setVisibility(View.GONE);
                    }
                } else {
                    lnNativeAd.setVisibility(View.GONE);
                }
    ...
    public void showNative(Activity activity, LinearLayout lnNative, int position, String idAds) {
        if (!PurchaseUtils.isNoAds(activity)) {
            if (!FirebaseQuery.getEnableAds(activity)) {
                if (FirebaseQuery.getEnableNative(activity)) {
                    loadNativeGA(activity, lnNative, position, idAds);
                } else {
                    NativeGAM.getInstance().showNativeGAM(activity, lnNative, position);
                }
            } else {
                if (lnNative != null) {
